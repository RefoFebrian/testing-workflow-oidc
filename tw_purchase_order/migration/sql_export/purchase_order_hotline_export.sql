...
SELECT
    ...
    , CASE WHEN rn = 1 THEN "No Hotline / External ID" END AS "No Hotline / External ID"
...
FROM (
    SELECT 
        ...
        , '__import__.part.hotline_' || part_hotline.transaction_id AS "No Hotline / External ID"
        ...
        -- , '[' || pp.default_code || '] ' || pt.name AS "Order Lines / Product"
        , '[' || pp.default_code || '] ' || pt.name AS "Order Lines / Product / Database ID"
    ...
    LEFT JOIN (
        SELECT
            tph.id AS transaction_id
        FROM teds_part_hotline tph
        WHERE 1=1
        AND tph.state = 'approved'
    ) part_hotline ON part_hotline.transaction_id = po.part_hotline_id
	...
	AND po.division = 'Sparepart'
	AND (picking.picking_ids IS NOT NULL OR invoice.invoice_ids IS NOT NULL)
    AND part_hotline.transaction_id IS NOT NULL
    AND wpot.name = 'Hotline'
) t;
...
WHERE 1=1
AND tpot.division = 'Sparepart' -- fill from "Division" PO Query TEDS
AND tpot.name = 'Hotline'; -- fill from "PO Type / Database ID" PO Query TEDS
...
AND (spt.division = 'Sparepart' OR spt.division IS NULL) -- fill from "Division" PO Query TEDS
...
WHERE 1=1
AND prod_tmpl.division = 'Sparepart' -- fill from "Division" PO Query TEDS
AND '[' || prod_tmpl.default_code || '] ' || COALESCE(prod_tmpl.name ->> 'en_US', prod_tmpl.description ->> 'en_US') ILIKE '%[EMBLEM,MOTOR COVER] 86110K4BD00%'; -- fill from "Order Lines / Product / Database ID" PO Query TEDS