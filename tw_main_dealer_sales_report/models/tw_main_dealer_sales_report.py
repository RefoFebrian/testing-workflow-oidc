...
class tw_main_dealer_sales_report(models.TransientModel):
    _name = "tw.main.dealer.sales.report"
    _description = "TW Main Dealer Sales Report"
    ...
    @api.model
    def _get_categ_ids(self, division):
        categ_obj = self.env['product.category']
        all_categ_ids = categ_obj.search([])
        return categ_obj.get_child_ids(all_categ_ids.ids, division)

    name = fields.Char('Filename', readonly=True)
    file = fields.Binary(string="File")
    options = fields.Selection([
        ('detail_per_type_warna','Detail per type warna'),
        ('detail_tanpa_hpp','Detail tanpa hpp')],string="Options" ,default="detail_per_type_warna", required=True)
    division = fields.Selection( [('Unit', 'Unit'), ('Sparepart', 'Sparepart')], default='Unit', string="Division" , required=True)
    ...
    state = fields.Selection([
        ...
        string='Customer AR State',
        required=True,
        default='all')
    type_file = fields.Selection([('excel', 'Excel'), ('csv', 'CSV')],string="Format File",default='excel')

    company_ids = fields.Many2many('res.company',string="Branch",domain=[('branch_type_id.value','=','MD')])
    product_ids = fields.Many2many('product.product','tw_main_dealer_sales_report_product_rel','tw_main_dealer_sales_report_id', 'product_id', 'Products')
    dealer_ids = fields.Many2many('res.partner', 'tw_main_dealer_sales_report_partner_rel', 'tw_main_dealer_sales_report_id', 'partner_id', 'Partners', copy=False, domain=[('category_id.name','in',['Dealer'])])
    
    def excel_report(self):
        self.env['web.report']._check_date_range_limit(self.start_date, self.end_date)

        if len(self.company_ids) == 0:
            user_company_ids = self.env.user._get_company_ids()
            # Filter hanya company dengan branch_type_id.value = 'MD'
            self.company_ids = self.env['res.company'].search([
                ('id', 'in', list(user_company_ids) if isinstance(user_company_ids, tuple) else user_company_ids.ids),
                ('branch_type_id.value', '=', 'MD')
            ])
        
        if self.options == 'detail_tanpa_hpp':
            return self._print_excel_report_non_hpp()
        else:
            return self._print_excel_report()
    
    def _print_excel_report(self, return_fp=False):
        ...
        query_group_by = " GROUP BY branch_code,branch_name,so.name,so.state,date_order,cust_code,cust_name,product_template,warna,qty,hpp,harga_jual,disc,harga_jual_excl_tax,total_hpp,no_invoice,nett_sales,discount_cash_avg,discount_lain_avg,discount_program_avg,dpp,tax,total,gp,gp_avg,category,parent_category,prod_series,faktur_pajak,soc_id,po_type"
        ...
        if self.division == 'Unit':
            ...
            SELECT 
                ...
                COALESCE(sol.cogs,0) * sol.product_uom_qty as total_hpp,
                ...
                ((sol.price_unit * (1 - COALESCE(sol.discount,0) / 100) / (1 + COALESCE(tax.amount,0)/100) * sol.product_uom_qty) - (COALESCE(sod.discount_cash,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_lain,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_program,0) / tent.total_qty * sol.product_uom_qty)) - COALESCE(sol.cogs,0) * sol.product_uom_qty as gp,
                (((sol.price_unit * (1 - COALESCE(sol.discount,0) / 100) / (1 + COALESCE(tax.amount,0)/100) * sol.product_uom_qty) - (COALESCE(sod.discount_cash,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_lain,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_program,0) / tent.total_qty * sol.product_uom_qty)) - COALESCE(sol.cogs,0) * sol.product_uom_qty) / sol.product_uom_qty as gp_avg,  
            ...
            SELECT 
                ...
                LEFT JOIN tw_faktur_pajak_out fp on so.faktur_pajak_out_id = fp.id
            %s %s %s
            """ % (query_where,query_group_by, query_order_by)
            ...
            if self.division == 'Unit':
                query_cancel = """
                    SELECT 
                        ...
                        (COALESCE(sol.cogs,0) * sol.product_uom_qty)* -1 as total_hpp,
                        ...
                        (((sol.price_unit * (1 - COALESCE(sol.discount,0) / 100) / (1 + COALESCE(tax.amount,0)/100) * sol.product_uom_qty) - (COALESCE(sod.discount_cash,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_lain,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_program,0) / tent.total_qty * sol.product_uom_qty)) - COALESCE(sol.cogs,0) * sol.product_uom_qty) *-1 as gp,
                        ((((sol.price_unit * (1 - COALESCE(sol.discount,0) / 100) / (1 + COALESCE(tax.amount,0)/100) * sol.product_uom_qty) - (COALESCE(sod.discount_cash,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_lain,0) / tent.total_qty * sol.product_uom_qty) - (COALESCE(sod.discount_program,0) / tent.total_qty * sol.product_uom_qty)) - COALESCE(sol.cogs,0) * sol.product_uom_qty) / sol.product_uom_qty)*-1 as gp_avg,  
                        COALESCE(prod_category.name,'') as category,
                        COALESCE(prod_category2.name,'') as parent_category,
                        COALESCE(series_id.name->>'en_US', series_id.name->>'id_ID') as prod_series,
                        invs.ref as no_invoice,
                ...
                query_cancel="""
                    SELECT 
                        ...
                        LEFT JOIN tw_faktur_pajak_out fp on so.faktur_pajak_out_id = fp.id
                    %s %s %s %s
                """ % (query_where_cancel, query_where, query_group_by, query_order_by)
        ...
        if self.env.context.get('api_rpa'):
            return query
        ...
        if self.type_file == 'excel':
            ...
            writer.writerow(["Branch Code", "Branch Name", "SO Number", "State", "Date", "Customer Code",
                                 "Customer Name", "Type", "Color", "Qty", "HPP", "Harga Jual", "Disc (%)",
                                 "Harga Jual Excl Tax", "Total HPP", "Sales", "Disc Cash (Avg)", "Disc Lain (Avg)",
                                 "Disc Program (Avg)", "DPP", "Tax", "Total Piutang", "Gross Profit",
                                 "Gross Profit (Avg)", "Category Name", "Parent Category Name", "Sales",
                                 "No Invoice", "Faktur Pajak"])
            ...
            for res in rows:
                ...
                date_order = datetime.strptime(res[5][0:22], "%Y-%m-%d %I:%M %p") if res[5] else ''
                cust_code = str(res[6].encode('ascii','ignore').decode('ascii')) if res[6] != None else ''
                cust_name = str(res[7].encode('ascii','ignore').decode('ascii')) if res[7] != None else ''
                type = str(res[8].encode('ascii','ignore').decode('ascii')) if res[8] != None else ''
                warna = str(res[9].encode('ascii','ignore').decode('ascii')) if res[9] != None else ''
                qty = res[10]
                hpp = res[11]
                harga_jual = res[12]
                disc = res[13]
                harga_jual_excl_tax = res[14]
                total_hpp = res[15]           
                nett_sales = res[16]
                ...
                no_invoice = str(res[28].encode('ascii','ignore').decode('ascii')) if res[28] != None else '' 
                faktur_pajak = str(res[29].encode('ascii','ignore').decode('ascii')) if res[29] != None else ''
                soc_id = res[30]
                ...
                writer.writerow([branch_code, branch_name, name, state, date_order,
                                     cust_code, cust_name, type, warna, qty, hpp, harga_jual,
                                     disc, harga_jual_excl_tax, total_hpp, nett_sales,
                                     discount_cash_avg, discount_lain_avg, discount_program_avg,
                                     dpp, tax, total, gp, gp_avg, categ_name, categ2_name,
            ...
            # csv_bytes = csv_bytes.encode('utf-8')x
