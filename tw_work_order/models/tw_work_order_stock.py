...
class TwWorkOrder(models.Model):
    """
    Work Order model that inherit from the tw.work.order model.
    """
    ...
    # 9: relation fields
    picking_ids = fields.One2many('stock.picking', 'work_order_id', string='Transfers')
    ...
    def _compute_warehouse_id(self):
        for order in self:
            default_warehouse_id = self.env['ir.default'].with_company(
                order.company_id.id)._get_model_defaults('tw.work.order').get('warehouse_id')
    ...
    def _log_decrease_ordered_quantity(self, documents, cancel=False):
        ...
        def _render_note_exception_quantity_so(rendering_context):
            ...
            order_line_ids = self.env['tw.work.order.line'].browse([order_line.id for order in order_exceptions.values() for order_line in order[0]])
            work_order_ids = order_line_ids.mapped('order_id')
...
class TwWorkOrderLine(models.Model):
    """
    Work Order model that inherits from the tw.work.order model.
    """
    _inherit = "tw.work.order.line"
    _description = "Work Order Line Stock"
    ...
    # 9: relation fields
    move_ids = fields.One2many('stock.move', 'work_order_line_id', string='Stock Moves')
    ...
    def _compute_qty_at_date(self):
        ...
        grouped_lines = defaultdict(lambda: self.env['tw.work.order.line'])
    ...
    def write(self, values):
        lines = self.env['tw.work.order.line']
        ...
        if lines and lines.division == 'Sparepart':
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        values.update({'work_order_line_id': self.id})
        return values
