WITH approval_unique AS (
    SELECT *,
           ROW_NUMBER() OVER (
               PARTITION BY division, branch_id, form_id, product_template_id 
               ORDER BY create_date DESC
           ) AS rn_hdr
    FROM wtc_approval_matrixdiscount_header
)
SELECT 
    ...
    'product' AS "Applied On",
    CASE WHEN line_data.rn = 1 THEN pt."name"  ELSE '' END as product_template,
...
INNER JOIN (
    SELECT 
        abl.wtc_approval_md_id ,
        abl.id as line_id,
        g.name as group_name,
        abl."limit",
        ROW_NUMBER() OVER (PARTITION BY abl.wtc_approval_md_id ORDER BY abl.id) as rn
    FROM wtc_approval_matrixdiscount abl
    LEFT JOIN res_groups g ON g.id = abl.group_id
) AS line_data ON line_data.wtc_approval_md_id = approval_biaya.id
inner join product_template pt on pt.id = approval_biaya.product_template_id 
