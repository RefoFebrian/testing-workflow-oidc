...
class SaleAdvancePaymentInv(models.TransientModel):
    _name = "tw.dealer.sale.advance.payment.inv"
    _inherit = "sale.advance.payment.inv"
    _description = "Dealer Sales Advance Payment Invoice"

    sale_order_ids = fields.Many2many('tw.dealer.sale.order', default=lambda self: self.env.context.get('active_ids'))
    ...
    def view_draft_invoices(self):
        return {
            ...
            'domain': [('line_ids.dealer_sale_order_line_ids.order_id', 'in', self.sale_order_ids.ids), ('state', '=', 'draft')],
        }
    ...
    def _create_invoices(self, sale_orders):
        ...
        if self.advance_payment_method == 'delivered':
            ...
            SaleOrderline = self.env['tw.dealer.sale.order.line'].with_context(sale_no_log_for_new_lines=True)
