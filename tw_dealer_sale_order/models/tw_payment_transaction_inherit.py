...
class PaymentTransaction(models.Model):
    ...
    dealer_sale_order_ids = fields.Many2many('tw.dealer.sale.order', string='Dealer Sales Orders', copy=False, readonly=True)
    dealer_sale_order_ids_nbr = fields.Integer(compute='_compute_dealer_sale_order_ids_nbr', string='# of Dealer Sales Orders')
    ...
    @api.depends('dealer_sale_order_ids')
    def _compute_dealer_sale_order_ids_nbr(self):
        for trans in self:
            trans.dealer_sale_order_ids_nbr = len(trans.dealer_sale_order_ids)
    ...
    def _post_process(self):
        ...
        for pending_tx in self.filtered(lambda tx: tx.state == 'pending'):
            ...
            sales_orders = pending_tx.dealer_sale_order_ids.filtered(
            ...
            if pending_tx.provider_id.code == 'custom':
                for order in pending_tx.dealer_sale_order_ids:
        ...
        for authorized_tx in self.filtered(lambda tx: tx.state == 'authorized'):
            ...
            if authorized_tx.operation == 'validation':
                continue
            if remaining_orders := (authorized_tx.dealer_sale_order_ids - confirmed_orders):
        ...
        for done_tx in self.filtered(lambda tx: tx.state == 'done'):
            ...
            if done_tx.operation == 'validation':
                continue
            (done_tx.dealer_sale_order_ids - confirmed_orders)._send_payment_succeeded_for_order_mail()

            auto_invoice = str2bool(
                self.env['ir.config_parameter'].sudo().get_param('sale.automatic_invoice')
            )
            if auto_invoice:
                # Invoice the dealer sales orders of confirmed transactions instead of only confirmed
    ...
    def _check_amount_and_confirm_order(self):
        """ Confirm the dealer sales order based on the amount of a transaction.

        Confirm the dealer sales orders only if the transaction amount (or the sum of the partial
        transaction amounts) is equal to or greater than the required amount for order confirmation

        Grouped payments (paying multiple dealer sales orders in one transaction) are not supported.

        :return: The confirmed dealer sales orders.
        :rtype: a `sale.order` recordset
        """
        
        model_name = 'tw.dealer.sale.order' if self.dealer_sale_order_ids else 'sale.order'
        confirmed_orders = self.env[model_name]
        for tx in self:
            # We only support the flow where exactly one quotation is linked to a transaction.
            if len(tx.dealer_sale_order_ids) == 1:
                quotation = tx.dealer_sale_order_ids.filtered(lambda so: so.state in ('draft', 'sent'))
    ...
    def _log_message_on_linked_documents(self, message):
        """ Override of payment to log a message on the dealer sales orders linked to the transaction.
        ...
        for order in self.dealer_sale_order_ids or self.source_transaction_id.dealer_sale_order_ids:
    ...
    def _cron_send_invoice(self):
        ...
        self.search([
            ...
            ('dealer_sale_order_ids.state', '=', 'sale'),
            ('last_state_change', '>=', retry_limit_date),
        ])._send_invoice()

    def _invoice_sale_orders(self):
        for tx in self.filtered(lambda tx: tx.dealer_sale_order_ids):
            tx = tx.with_company(tx.company_id)

            confirmed_orders = tx.dealer_sale_order_ids.filtered(lambda so: so.state == 'sale')
            if confirmed_orders:
    ...
    def _compute_reference_prefix(self, provider_code, separator, **values):
        ...
        If the `values` parameter has an entry with 'dealer_sale_order_ids' as key and a list of (4, id, O)
        or (6, 0, ids) X2M command as value, the prefix is computed based on the dealer sales order name(s)
        Otherwise, the computation is delegated to the super method.

        :param str provider_code: The code of the provider handling the transaction
        :param str separator: The custom separator used to separate data references
        :param dict values: The transaction values used to compute the reference prefix. It should
                            have the structure {'dealer_sale_order_ids': [(X2M command), ...], ...}.
        :return: The computed reference prefix if order ids are found, the one of `super` otherwise
        :rtype: str
        """
        command_list = values.get('dealer_sale_order_ids')
        if command_list:
            # Extract dealer sales order id(s) from the X2M commands
            order_ids = self._fields['dealer_sale_order_ids'].convert_to_cache(command_list, self)
    ...
    def action_view_sales_orders(self):
        ...
        dealer_sale_order_ids = self.dealer_sale_order_ids.ids
        if len(dealer_sale_order_ids) == 1:
            action['res_id'] = dealer_sale_order_ids[0]
            action['view_mode'] = 'form'
        else:
            action['view_mode'] = 'list,form'
            action['domain'] = [('id', 'in', dealer_sale_order_ids)]
        return action
