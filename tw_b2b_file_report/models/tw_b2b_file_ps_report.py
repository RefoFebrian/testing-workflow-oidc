...
class TWB2BFilePSReport(models.TransientModel):
    _name = "tw.b2b.file.ps.report"
    _description = "Laporan B2B File PS"
    ...
    def action_generate_report(self):
        ...
        # Build status filter condition based on self.state
        status_filter = ""
        if self.state:
            if self.state == 'outstanding':
                status_filter = "WHERE status IN ('on intransit', 'packed at ahm')"
            else:
                status_filter = f"WHERE status = '{self.state}'"
        ...
        file_name = f"report_file_ps_{datetime.now().strftime('%Y-%m-%d')}.xlsx"
        if ress:
