...
class PrintPartSalesCancelPrint(models.AbstractModel):
    _name = "report.tw_part_sales_cancel.tw_part_sales_cancel_print"
    _description = "Part Sales Cancel Print"
    ...
    def dealer_address(self):
        part_sales_cancel_obj = self.env['tw.part.sales.cancel'].suspend_security().browse(self.env.context.get('active_ids', []))
        dealer_name = ["", "", "", "", "", "", "", "", ""]
        if part_sales_cancel_obj.partner_id:
            dealer_name = [
                part_sales_cancel_obj.partner_id.name,
                part_sales_cancel_obj.partner_id.street,
                part_sales_cancel_obj.partner_id.rt,
                part_sales_cancel_obj.partner_id.rw,
                part_sales_cancel_obj.partner_id.sub_district_id.name,
                part_sales_cancel_obj.partner_id.district_id.name,
                part_sales_cancel_obj.partner_id.city_id.name,
                part_sales_cancel_obj.partner_id.state_id.name,
                part_sales_cancel_obj.partner_id.mobile
            ]
        return dealer_name
    ...
    def invoice_name(self):
        ...
        move_obj = self.env['account.move'].suspend_security().search([
            ('ref', 'ilike', part_sales_cancel_obj.name), 
    ...
    def print_counter(self):
        model_id = self.env.ref('tw_part_sales_cancel.model_tw_part_sales_cancel').id
        report_id = self.env.ref('tw_part_sales_cancel.tw_part_sales_cancel_print').id
