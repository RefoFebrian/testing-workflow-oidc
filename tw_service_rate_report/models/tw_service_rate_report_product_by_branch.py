...
class ServiceRateReportProductByBranch(models.TransientModel):
    _inherit = "tw.service.rate.report"
    _description = "Service Rate Report Product By Branch"


    def get_data_result(self):
        result = super().get_data_result()
        data_product_by_branch = self._get_data_sr_product_by_branch()
        result['Product By Branch'] = data_product_by_branch

        return result

    def _get_data_sr_product_by_branch(self):
        if self.start_date:
        ...
        query = """
            SELECT 
                ...
                END AS "PO x SJ"
            FROM (
                --SERVICE RATE by Product per Branch
                SELECT
                ...
                order by branch.code, product.default_code 
            ) AS service_rate
