...
class ServiceRateReportCategory(models.TransientModel):
    _inherit = "tw.service.rate.report"
    _description = "Service Rate Report Category"

    def get_data_result(self):
        result = super().get_data_result()
        data_category = self._get_data_sr_category()
        result['Category'] = data_category

        return result

    def _get_data_sr_category(self):
        if self.start_date:
