...
class TWBoomDataStockOutstandingBPKB(models.Model):
    _inherit = "tw.boom.task"
    _description = "TW Boom Data STOCK Outstanding BPKB"
    _order = "id desc"
    ...
    # 14: private methods
    def _get_data_stock_outstanding_bpkb_current(self, limit=100, start_date=None, end_date=None, branch_code=None, source_transaksi=None, no_transaksi=None):
        query_where = ""
        query_limit = ""
        query_date = ""
        query_branch = ""
        ...
        query = """
            SELECT
                ...
                , 'Outstanding BPKB (Belum Terima)' AS kategori
                , NULL AS product_id
                ...
                , CASE WHEN lot.vehicle_ownership_receipt_date IS NOT NULL THEN to_char(bpkb.confirm_date,'YYYY-MM-DD HH24:MI:SS') ELSE NULL END AS done_date
                , CASE 
                    WHEN lot.vehicle_ownership_receipt_date IS NULL THEN 'open' 
                    ELSE 'done' 
                END AS state
            FROM stock_lot lot
            ...
            left join lateral (
                ...
                AND tbc.name = 'Outstanding BPKB (Belum Terima)' 
                AND boom.state = 'open'
            ) tab on True
            WHERE 1=1
            ...
            AND (lot.vehicle_ownership_receipt_date is NULL OR lot.vehicle_ownership_receipt_date >= (NOW() - INTERVAL '1 DAYS')::DATE)
            {query_date}
            {query_branch}
            {query_where}
            AND NOT EXISTS (
                ...
                WHERE boom.no_transaction = dso.name 
                AND boom.source_transaction = lot.name
                AND boom.state = CASE WHEN lot.vehicle_ownership_receipt_date IS NULL THEN 'open' ELSE 'done' END
            )
            {query_limit}
        """.format(
