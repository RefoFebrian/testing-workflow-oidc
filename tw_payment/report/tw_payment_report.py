...
class CustomerPaymentKwitansiReport(models.AbstractModel):
    _name = "report.tw_payment.report_tw_account_payment_kwitansi"
        ...
        if transaction_type == 'reference':
            if len(payment_line) > 1:
                for pl in payment_line:
                    reference += f"{pl.move_line_id.name or pl.payment_id.memo}, "
            else:
                return payment_line.move_line_id.name or payment_line.payment_id.memo
