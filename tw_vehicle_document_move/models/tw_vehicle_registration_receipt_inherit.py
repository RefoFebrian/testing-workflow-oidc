# -*- coding: utf-8 -*-
from datetime import datetime
from odoo import api, fields, models, _


class TwVehicleRegistrationReceiptInherit(models.Model):
    _inherit = "tw.vehicle.registration.receipt"

    def action_confirm(self):
        """Override to create document move tracking after confirmation"""
        res = super(TwVehicleRegistrationReceiptInherit, self).action_confirm()
        
        # Create document move for each STNK received
        move_model = self.env['tw.vehicle.document.move']
        for rec in self.filtered(lambda r: r.state == 'done'):
            for line in rec.vehicle_registration_receipt_line_ids:
                if line.vehicle_registration_number and not line.vehicle_registration_received:
                    move_model._create_document_move({
                        'reference': rec.name,
                        'date': rec.date,
                        'document_type': 'vehicle_registration',
                        'document_number': line.vehicle_registration_number,
                        'lot_id': line.lot_id.id,
                        'destination_location_id': rec.vehicle_registration_location_id.id,
                        'company_id': rec.company_id.id,
                    })
        
        return res

    def action_cancel(self):
        """Override to create document move tracking on cancel (STNK receipt)"""
        move_model = self.env['tw.vehicle.document.move']
        for rec in self.filtered(lambda r: r.state == 'done'):
            for line in rec.vehicle_registration_receipt_line_ids:
                if line.vehicle_registration_number:
                    move_model._create_document_move({
                        'reference': f"{rec.name} (CANCEL)",
                        'date': fields.Date.context_today(self),
                        'document_type': 'vehicle_registration',
                        'document_number': line.vehicle_registration_number,
                        'lot_id': line.lot_id.id,
                        'source_location_id': line.lot_id.vehicle_registration_location_id.id,
                        # destination kosong - dokumen kembali ke intransit
                        'company_id': rec.company_id.id,
                    })

        return super(TwVehicleRegistrationReceiptInherit, self).action_cancel()
