...
class TWBoomDataARWOReg(models.Model):
    _inherit = "tw.boom.task"
    _description = "TW Boom Data AR WO Reguler"
    _order = "id desc"
    ...
    # 14: private methods
    def schedule_generate_data_ar_part_wo_reg(self, limit=100, start_date=None, end_date=None, branch_code=None, source_transaksi=None, no_transaksi=None):
        query_where = ""
        query_limit = ""
        query_select = ""
        query_date = ""
        ...
        query = """
            SELECT
                b.id AS company_id
                , 'Pelunasan WO Reguler' AS kategori"
                , NULL as finco
            ...
            AND wo.type = 'REG'
            AND aj.name->>'en_US' = 'Journal WO Reguler'
            {query_where}
            {query_date}
            {query_branch}
            {query_limit}
        """.format(
            ...
            query_where_boom=query_where_boom
        )
