...
class TWBoomDataARSL(models.Model):
    _inherit = "tw.boom.task"
    _description = "TW Boom Data AR SL"
    _order = "id desc"
    ...
    # 14: private methods
    def schedule_generate_data_ar_unit_sl(self, limit=100, start_date=None, end_date=None, branch_code=None, source_transaksi=None, no_transaksi=None):
        query_where = ""
        query_limit = ""
        query_date = ""
        query_branch = ""
        ...
        if branch_code:
            ...
            if list_branch_pilot:
                query_branch += "AND b.id IN (%s)" % ",".join(map(str, list_branch_pilot))
        ...
        query = """
            SELECT
                b.id AS company_id
                , 'Pelunasan Unit (SL)' AS kategori
                , pp.id AS product_id
                , pav.id AS product_color_id
                , lot.id AS lot_id
                , finco.id AS finco_id
            ...
            AND aj.name->>'en_US' = 'Journal Pelunasan SO'
            {query_where}
            {query_branch}
            {query_date}
