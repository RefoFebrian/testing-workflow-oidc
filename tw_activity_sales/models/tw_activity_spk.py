...
class TWActivitySPK(models.Model):
    ...
    def _get_sale_order(self):
        so = super(TWActivitySPK,self)._get_sale_order()
        so['jaringan_penjualan'] = self.jaringan_penjualan
        so['sumber_penjualan_id'] = self.sumber_penjualan_id.id
        so['activity_plan_id'] = self.activity_plan_id.id
        return so

    def _prepare_dealer_sale_order_vals(self):
        vals = super(TWActivitySPK,self)._prepare_dealer_sale_order_vals()
        if self.activity_plan_id:
            vals.update({
                'sales_channel_id': self.activity_plan_id.sales_channel_id.id,
                'sales_source_location_id': self.activity_plan_id.location_id.id,
