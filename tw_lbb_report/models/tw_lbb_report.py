...
class TwLBBReport(models.TransientModel):
    _name = "tw.lbb.report"
    _description = "TW LBB Report"
    ...
    options = fields.Selection(
        [
            ('lbb','LBB'),
            ('wpp','WPP'),
            ('persentasi_mekanik','Prestasi Mekanik'),
            ('detil_prestasi','Detil Prestasi Mekanik')
        ],
        string="Options",
        required=True,
        default='lbb'
    ...
    def _onchange_date(self):
        if self.start_date and self.end_date:
            if self.start_date > self.end_date:
                raise Warning(_("Tanggal Mulai tidak boleh melewati Tanggal Akhir"))
    ...
    def excel_report(self):
        ...
        report_id, file_name = (False, False)
        if self.options == 'lbb' : 
            report_id, file_name = self._print_excel_report_lbb()
        if self.options == 'wpp' : 
            report_id, file_name = self._print_excel_report_wpp()
        if self.options == 'persentasi_mekanik' : 
            report_id, file_name = self._print_excel_report_prestasi_mekanik()
        if self.options == 'detil_prestasi' :   
            # web report
            ress = self._print_excel_report_prestasi_mekanik_detil()
            return self.env['web.report'].sudo().generate_report('Report Prestasi Mekanik Detil',ress, data_summary_header=False, start_date=self.start_date, end_date=self.end_date)

        if report_id and file_name:
            self.write({
                'name': file_name,
            })
            ...
            return {
                ...
                "target": "new",
                'url': '/web/content/web.report/%s/report_file/%s?download=true' % (report_id.id, file_name)
            }

    def _print_excel_report_lbb(self):
        fp = BytesIO()
        ...
        query_sum = f"""
            SELECT 
            wo.company_id
            ,COALESCE(SUM(CASE WHEN wol.division = 'Service' AND pc2.name = 'KPB' THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) total_kpb
            ,COALESCE(SUM(CASE WHEN wol.division = 'Service' AND (pc2.name = 'QS' OR pc.name in ('LR', 'HR')) THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) total_qs_lr_hr
            ,COALESCE(SUM(CASE WHEN wol.division = 'Service' AND pc.name = 'CLA' THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) total_claim, 0 AS total_lain
            ,COALESCE(SUM(CASE WHEN wol.division = 'Service' THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) amt_jasa
            ,COALESCE(SUM(CASE WHEN wol.division = 'Sparepart' AND pc.name NOT IN ( 'OLI','OIL','TIRE','TIRE1') THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) amt_part
            ,COALESCE(SUM(CASE WHEN wol.division = 'Sparepart' AND pc.name IN ( 'OLI','OIL') THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) amt_oil
            ,COALESCE(SUM(CASE WHEN wol.division = 'Sparepart' AND pc.name IN ('TIRE','TIRE1') THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END),0) amt_tire
            ,COALESCE( SUM(CASE WHEN pc.name in ('OLI','OIL') and wol.division='Sparepart' then wol.qty_delivered END),0)as count_oli
            ,COALESCE( SUM(CASE WHEN pc.name in ('TIRE','TIRE1') and wol.division='Sparepart' then wol.qty_delivered END),0)as count_tire
            ...
            WHERE wo.date BETWEEN '{start_date}' AND '{end_date}'
            AND wo.company_id = {company_id}
            GROUP BY wo.company_id
        """ 
            
        self._cr.execute (query_sum)
        ...
        query = f"""
            ...
            wo_header.nama_unit->>'en_US' as nama_unit,
            wo_header.code_unit->>'en_US' as code_unit,
            wo_header.categ,
            ...
            COUNT(CASE WHEN type.value='CLA' then wo.id END ) as total_claim,
            COUNT(CASE WHEN type.value='WAR' then wo.id END ) as total_jr,
            COUNT(wo.id) as total_unit
            FROM tw_work_order as wo
            ...
            LEFT JOIN res_company as branch_header ON branch_header.id=wo.company_id
            where wo.date >= '{start_date}' and wo.date<= '{end_date}'
            AND wo.company_id= {company_id} 
            ...
            COUNT(CASE WHEN prc.name in ('CS') and wo_line.division='Service' then wo_detail_line.id END ) as total_cs,
            COUNT(CASE WHEN prc.name in ('LS') and wo_line.division='Service' then wo_detail_line.id END ) as total_ls,
            COUNT(CASE WHEN prc.name in ('OR+') and wo_line.division='Service' then wo_detail_line.id END ) as total_or,
            COUNT(CASE WHEN prc.name in ('LR') and wo_line.division='Service' then wo_detail_line.id END ) as total_lr,
            COUNT(CASE WHEN prc.name in ('HR') and wo_line.division='Service' then wo_detail_line.id END ) as total_hr,
            COUNT(wo_line.id ) as total_pekerjaan
            FROM tw_work_order as wo_detail_line
            LEFT JOIN tw_work_order_line as wo_line ON wo_detail_line.id=wo_line.order_id
            ...
            LEFT JOIN res_company as branch_detail ON branch_detail.id=wo_detail_line.company_id
            WHERE wo_detail_line.date >= '{start_date}' and wo_detail_line.date <= '{end_date}'
            AND wo_detail_line.company_id = {company_id} 
            ...
            FROM tw_work_order as wo_entry
            LEFT JOIN tw_selection type on type.id = wo_entry.type_id
            LEFT JOIN stock_lot as lot ON lot.id=wo_entry.lot_id
            ...
            LEFT JOIN res_company as branch_header_entry  ON branch_header_entry.id=wo_entry.company_id
            WHERE type.value <> 'WAR' AND type.value <> 'SLS' 
            AND wo_entry.date >= '{start_date}' and wo_entry.date <= '{end_date}'
            AND wo_entry.company_id = {company_id} 
        ...
        file_name = 'Laporan LBB '+str(date)+'.xlsx'        
        ...
        worksheet.merge_range('G%s:J%s' % (2,2), self.company_id.atpm_code if self.company_id.atpm_code else '', wbf['title_2']) 
        ...
        worksheet.merge_range('G%s:J%s' % (3,3), self.company_id.name if self.company_id.name else '', wbf['title_2']) 
        ...
        worksheet.merge_range('G%s:J%s' % (4,4), self.company_id.city_id.name if self.company_id.city_id.name else '', wbf['title_2']) 
        ...
        for res in ress:
            id_unit  = res[0]
            description_unit = str(res[1]) if res[1] != None else ''
            code_unit = str(res[2]) if res[2] != None else ''
            category = str(res[3]) if res[3] != None else ''
            category_parent = str(res[4]) if res[4] != None else ''
            count_kpb1 = res[5]
            count_kpb2 = res[6]
            count_kpb3 = res[7]
            count_kpb4 = res[8]
        ...
        for ress_matic in ress:
            ...
            description_unit = str(ress_matic[1]) if ress_matic[1] != None else ''
            code_unit = str(ress_matic[2]) if ress_matic[2] != None else ''
            category = str(ress_matic[3]) if ress_matic[3] != None else ''
            category_parent = str(ress_matic[4]) if ress_matic[4] != None else ''
        ...
        for ress_sport in ress:
            ...
            description_unit = str(ress_sport[1]) if ress_sport[1] != None else ''
            code_unit = str(ress_sport[2]) if ress_sport[2] != None else ''
            category = str(ress_sport[3]) if ress_sport[3] != None else ''
            category_parent = str(ress_sport[4]) if ress_sport[4] != None else ''
