...
class TwWPPReport(models.TransientModel):
    ...
    # 13: Action Methods
    def _print_excel_report_wpp(self):
        fp = BytesIO()
        ...
        file_name = 'Workshop  Performance Parameter.xlsx' 
        ...
        query = f"""
            ...
            FROM
                ...
                , COUNT(CASE WHEN type.value <>  'WAR' THEN wo.id END) AS cnt_not_war
                FROM tw_work_order wo
                LEFT JOIN tw_selection type on type.id = wo.type_id
                WHERE wo.date BETWEEN '{start_date}' AND '{end_date}'
                GROUP BY wo.company_id) AS wo_inv
            FULL OUTER JOIN
                (SELECT company_id, SUM(cnt_per_date) AS unit_entry
                FROM (
                SELECT wo.company_id, wo.date, COUNT(DISTINCT lot_id) AS cnt_per_date
                FROM tw_work_order wo
                LEFT JOIN tw_selection type on type.id = wo.type_id
                WHERE type.value <> 'WAR' AND type.value <> 'SLS' 
                AND wo.date BETWEEN '{start_date}' AND '{end_date}'
                GROUP BY wo.company_id, wo.date) wo_per_date
                GROUP BY company_id) AS wo_unit
            ON wo_inv.company_id = wo_unit.company_id
            FULL OUTER JOIN
                (SELECT  
                    hr_sales.company_id as company_id
                    ...
                    LEFT JOIN tw_selection sf on sf.id = job.sales_force_id
                    WHERE sf.name='Mekanik'
                    and hr_sales.company_id={self.company_id.id}
                    and job.name->>'en_US' <> 'KEPALA MEKANIK'
                    GROUP BY hr_sales.company_id
                ) as total_mekanik
            ON total_mekanik.company_id = wo_unit.company_id
            --FULL OUTER JOIN
                --(SELECT 
                    ...
                    --from tw_attendance as absensi
                    --LEFT JOIN hr_employee hr_sales ON absensi.nip = hr_sales.nip 
                    --LEFT JOIN res_company as branch ON branch.id=hr_sales.company_id
                    --where branch.id={self.company_id.id} AND absensi.bulan='{start_date[:-3]}'
                    --GROUP BY branch.id,absensi.jumlah_hari_kerja
                --) as absensi_mekanik
            --ON absensi_mekanik.company_id = total_mekanik.company_id
            FULL OUTER JOIN
                (SELECT
                        wo.company_id 
                        ,date_part( 'epoch',  SUM(age(wo_start.finish_date,wo_start.start_date) )::interval   )/3600 as jam_terpakai
                        from tw_work_order as wo
                        LEFT JOIN tw_start_stop_wo AS wo_start
                        ON wo.id=wo_start.work_order_id 
                        LEFT JOIN res_company as branch ON branch.id=wo.company_id
                        where 1=1
                        and  wo.date BETWEEN '{start_date}' AND '{end_date}'
                        GROUP BY wo.company_id
                ) as jam_terpakai
            ON jam_terpakai.company_id = total_mekanik.company_id
            FULL OUTER JOIN
                (SELECT wo.company_id
                , SUM(CASE WHEN wol.division = 'Service' THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END) amt_jasa
                , SUM(CASE WHEN wol.division = 'Sparepart' AND pc.name NOT IN ( 'OLI','OIL','TIRE','TIRE1') THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END) amt_part
                , SUM(CASE WHEN wol.division = 'Sparepart' AND pc.name = 'OIL' THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END) amt_oil
                , SUM(CASE WHEN wol.division = 'Sparepart' AND pc.name in ( 'TIRE' ,'TIRE1') THEN wol.price_unit*(1-COALESCE(wol.discount,0)/100) END) amt_tire
            FROM tw_work_order wo
            ...
            WHERE wo.date BETWEEN '{start_date}' AND '{end_date}'
            GROUP BY wo.company_id) AS wo_jasa
            ON wo_inv.company_id = wo_jasa.company_id
            INNER JOIN res_company b ON b.id = COALESCE(wo_inv.company_id, COALESCE(wo_unit.company_id, wo_jasa.company_id,total_mekanik.company_id ))
            where 1=1 AND b.id={self.company_id.id}
        """
        ...
        worksheet.write('B%s' % (5), self.company_id.atpm_code , wbf['title_2'])
        worksheet.write('B%s' % (6), name_branch , wbf['title_2'])
        worksheet.write('B%s' % (7), self.company_id.city_id.name, wbf['title_2'])
