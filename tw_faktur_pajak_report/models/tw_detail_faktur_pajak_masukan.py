...
class TWDetailFakturPajakMasukan(models.TransientModel):
    """Wizard for generating Detail Faktur Pajak Masukan report."""
    ...
    def action_generate_report(self):
        """Generate the Detail Faktur Pajak Masukan Excel report."""
        ...
        if self.start_date and self.end_date:
            query_where += " AND tpr.date_faktur_pajak BETWEEN '%s' AND '%s'" % (
        ...
        query = """
            ...
            LEFT JOIN account_tax tax ON tax.id = COALESCE(tax_rel.account_tax_id)
        ...
        if not ress:
            raise UserError('Data tidak ada...')

        report_name = "Laporan Detail Faktur Pajak Masukan"
    ...
    def _get_summary_header_data(self):
        """Prepare summary header data for Excel report."""
        company_name = self.company_id.name
        return {
            "A3": company_name if self.company_id else '-',
            "A4": "Detail Faktur Pajak Masukan Periode : %s s/d %s" % (
