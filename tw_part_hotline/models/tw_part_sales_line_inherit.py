...
class TwPartSalesLine(models.Model):
    ...
    part_hotline_id = fields.Many2one(
        'tw.part.hotline',
        string="No Hotline",
        domain="[('customer_id', '=', parent.partner_id), ('company_id', '=', parent.company_id), ('state', '=', 'approved'), ('status_po', '=', 'done'), ('is_used', '=', False)]"
    )
    ...
    def _onchange_part_hotline_id(self):
        """When a hotline is selected on a PS line, populate product from hotline detail."""
        ...
        categ_sparepart = self.env['product.category'].search([('name', '=', 'Sparepart')], limit=1)
