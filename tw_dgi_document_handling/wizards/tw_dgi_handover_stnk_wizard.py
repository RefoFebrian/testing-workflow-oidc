...
class TWDGIHandoverStnkWizard(models.TransientModel):
    """Wizard for syncing Penyerahan STNK data from DGI API.
    ...
    Output Template:
        {"id_spk": "idSPK", "receiver": "namaPenerima", "registration_handover_line_ids": {"lot_id": "unit.nomorRangka"}}
    """
    _name = "tw.dgi.handover.stnk.wizard"
    _inherit = "tw.dgi.wizard.mixin"
    _description = "DGI Handover STNK Wizard"

    def action_get_dgi_data(self):
        """Main action: GET data dari DGI dan process"""
        return self.action_sync_dgi_data(endpoint_code='doch_handover_stnk')
    ...
    def _get_item_identifier(self, endpoint, item):
        ...
        if id_spk:
            return f"Penyerahan STNK SPK {id_spk}"
    ...
    def _create_record_from_response(self, endpoint, vals):
        """Create Penyerahan STNK dari engine-parsed values.

        Engine sudah handles:
        - Filtering by statusFakturSTNK = 4
        - Mapping unit.nomorRangka to lot_id (via relation mapping with idSPK)
        - Extracting namaPenerima -> receiver
        """
        model = self.env['tw.vehicle.registration.handover'].sudo()
        line_items = vals.get('registration_handover_line_ids', [])
        ...
        for line_item in line_items:
            ...
            # Validasi lot state
            if not self._validate_lot_handover_stnk(lot):
                error_lots.append(lot.name or str(lot_id))
                continue

            # Build line vals
            stnk_handover_date = line_data.get('stnk_handover_date')
            line_vals = {
                'lot_id': lot.id,
                'stnk_handover_date': stnk_handover_date,
                'plate_handover_date': stnk_handover_date,
            }
            ...
            if cust_id not in handover_vals:
                handover_vals[cust_id] = {
                    ...
                    'registration_handover_line_ids': [Command.create(line_vals)],
                }
            else:
                handover_vals[cust_id]['registration_handover_line_ids'].append(
        ...
        created_records = self._create_grouped_records(model, handover_vals, 'Penyerahan STNK')
        ...
        if created_records:
            extra_logs.append(f"Created {len(created_records)} transaksi Penyerahan STNK")
            return created_records.with_context(dgi_success_log_lines=extra_logs)

        raise UserError("Gagal membuat transaksi Penyerahan STNK")

    def _validate_lot_handover_stnk(self, lot):
        """Validasi lot state untuk Penyerahan STNK."""
        # Check branch
        ...
        # Check pajak progressive (jika ada)
        if self.company_id.branch_setting_id.pajak_progressive and not lot.registration_billing_id:
            self._add_process_log(
                f"NoRangka {lot.name}: Belum diproses di Tagihan Birojasa", 'ERROR')
            return False

        # Check sudah terima STNK
        if not lot.notice_receipt_date and not lot.vehicle_registration_receipt_date:
            self._add_process_log(
                f"NoRangka {lot.name}: Belum diproses di Penerimaan STNK", 'ERROR')
            return False

        # Check existing handover
        existing = self.env['tw.vehicle.registration.handover.line'].sudo().search([
            ('lot_id', '=', lot.id),
        ...
        if existing:
            self._add_process_log(
                f"NoRangka {lot.name}: Sudah ada di {existing.vehicle_registration_handover_id.name}", 'ERROR')
            return False

        # Check sudah selesai
        if lot.registration_handover_date and lot.plate_handover_date:
            self._add_process_log(
    ...
    def _create_grouped_records(self, model, grouped_vals, process_name):
        ...
        created = self.env['tw.vehicle.registration.handover']
